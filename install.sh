    ...
    if $SHELL_CHOICE; then
        ...
        case $SHELL_CHOICE in
            ...
            "zsh")
                ...
                execute_command "curl -fLo \"$ZSHRC\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/zshrc" "Default configuration" || error_msg "Default configuration impossible"
                ...
                if $USE_GUM; then
                    if gum_confirm "Install PowerLevel10k ?"; then
                        ...
                        if gum_confirm "Install custom prompt ?"; then
                            execute_command "curl -fLo \"$HOME/.p10k.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/p10k.zsh" "Custom prompt installation" || error_msg "Custom prompt installation impossible"
                    ...
                    if [[ "$CHOICE" =~ ^[yY]$ ]]; then
                        ...
                        tput el
                        if [[ "$CHOICE" =~ ^[yY]$ ]]; then
                            execute_command "curl -fLo \"$HOME/.p10k.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/p10k.zsh" "OhMyTermux prompt installation" || error_msg "OhMyTermux prompt installation impossible"
                            echo -e "\n# For customizing the prompt, run \`p10k configure\` or edit ~/.p10k.zsh." >> "$ZSHRC"
                            echo "[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh" >> "$ZSHRC"
                        else
                            echo -e "${COLOR_BLUE}You can customize the prompt by running 'p10k configure' or editing ~/.p10k.zsh.${COLOR_RESET}"
                ...
                execute_command "(curl -fLo \"$HOME/.oh-my-zsh/custom/aliases.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/aliases.zsh && \
                                ...
                                curl -fLo \"$HOME/.config/OhMyTermux/help.md\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/help.md)" "Default configuration" || error_msg "Default configuration impossible"
...
install_xfce() {
    if $XFCE_CHOICE; then
        ...
        execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/xfce.sh" "Downloading XFCE script" || error_msg "Impossible to download XFCE script"
    ...
    if $PROOT_CHOICE; then
        ...
        if $USE_GUM; then
            if gum confirm --affirmative "Yes" --negative "No" --prompt.foreground="33" --selected.background="33" "Install Debian Proot ?"; then
                ...
                execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/proot.sh" "Downloading Proot script" || error_msg "Impossible to download Proot script"
...
install_utils() {
    ...
    execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/utils.sh" "Downloading Utils script" || error_msg "Impossible to download Utils script"
