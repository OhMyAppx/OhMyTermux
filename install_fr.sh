    ...
    if $SHELL_CHOICE; then
        ...
        case $SHELL_CHOICE in
            ...
            "zsh")
                ...
                execute_command "curl -fLo \"$ZSHRC\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/zshrc" "Configuration par défaut" || error_msg "Configuration par défaut impossible"
                ...
                if $USE_GUM; then
                    if gum_confirm "Installer PowerLevel10k ?"; then
                        ...
                        if gum_confirm "Installer le prompt personnalisé ?"; then
                            execute_command "curl -fLo \"$HOME/.p10k.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/p10k.zsh" "Installation du prompt personnalisé" || error_msg "Impossible d'installer le prompt personnalisé"
                    ...
                    if [[ "$CHOICE" =~ ^[oO]$ ]]; then
                        ...
                        tput el
                        if [[ "$CHOICE" =~ ^[oO]$ ]]; then
                            execute_command "curl -fLo \"$HOME/.p10k.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/p10k.zsh" "Installation du prompt OhMyTermux" || error_msg "Impossible d'installer le prompt OhMyTermux"
                            echo -e "\n# Pour personnaliser le prompt, exécuter \`p10k configure\` ou éditer ~/.p10k.zsh." >> "$ZSHRC"
                            echo "[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh" >> "$ZSHRC"
                        else
                            echo -e "${COLOR_BLUE}Vous pouvez configurer le prompt en exécutant 'p10k configure'.${COLOR_RESET}"
                ...
                execute_command "(curl -fLo \"$HOME/.oh-my-zsh/custom/aliases.zsh\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/aliases.zsh && \
                                mkdir -p $HOME/.config/OhMyTermux && \
                                curl -fLo \"$HOME/.config/OhMyTermux/help.md\" https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/src/help.md)" "Configuration par défaut" || error_msg "Configuration par défaut impossible"
...
install_xfce() {
    if $XFCE_CHOICE; then
        ...
        execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/xfce_dev.sh" "Téléchargement du script XFCE" || error_msg "Impossible de télécharger le script XFCE"
    ...
    if $PROOT_CHOICE; then
        ...
        if $USE_GUM; then
            if gum confirm --affirmative "Oui" --negative "Non" --prompt.foreground="33" --selected.background="33" "Installer Debian Proot ?"; then
                ...
                execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/proot_dev.sh" "Téléchargement du script Proot" || error_msg "Impossible de télécharger le script Proot"
...
install_utils() {
    ...
    execute_command "curl -O https://raw.githubusercontent.com/OhMyAppx/OhMyTermux/main/utils_fr.sh" "Téléchargement du script Utils" || error_msg "Impossible de télécharger le script Utils"
...
rm -f xfce_fr.sh proot_fr.sh utils_fr.sh install_fr.sh >/dev/null 2>&1
